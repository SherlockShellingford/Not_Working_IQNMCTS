...
from dopamine.discrete_domains.mcts import *
import numpy as np
...
class Runner(object):
  ...
  def _run_one_episode(self, run_mode_str):
    ...
    count=0
    node=TreeNode()
    mcts=MonteCarloTreeSearch(self._agent)
    ...
    #Player1
      ...
      #Player2
        if run_mode_str=='train1':
          ...
          action2=random.choice(self._environment.available_actions())
          best_child=node.get_child_from_action(action2)
          pi=deepcopy(best_child.parent.child_psas)
          observation2, reward2, is_terminal = self._run_one_step(action2)
          
        elif run_mode_str=='eval':
          action2=self.opponent.act(originalobservation, self._environment.available_actions())
          best_child=node.get_child_from_action(action2)
          pi=deepcopy(best_child.parent.child_psas)
          observation2, reward2, is_terminal = self._run_one_step(action2)
        elif run_mode_str=='train2':
          ...
          print("Marco polo2")
          action2 = self._agent.get_action_from_observation(observation)
          observation2, reward2, is_terminal = self._run_one_step(action2)
        node=best_child
    ...
    if run_mode_str=='trainrandom':
      for item in gameplayer:
          self._agent._store_transition([item[0]], item[3], item[1], item[2],item[4])
          if not item[2]:
