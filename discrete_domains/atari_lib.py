...
from dopamine.discrete_domains.CGF import *
import atari_py
import gym
...
def implicit_quantile_network(num_actions, quantile_embedding_dim,
                              network_type, state, num_quantiles,training):
  ...
  relu4_flat = tf.reshape(relu4, [-1, row * column * 2])
  logits = tf.layers.dense(inputs=relu4_flat, units=action_size)
  ...
  relu5_flat = tf.reshape(relu5, [-1, action_size])
  ...
  v = tf.nn.tanh(dense2)
  # Loss Function
  print ('state_net:', v.shape, ", num_quan:", num_quantiles)
  state_net_size = v.get_shape().as_list()[-1]

  state_net_tiled = tf.tile(v, [num_quantiles, 1])
  ...
  pi_2 = tf.constant(math.pi)
  quantile_net = tf.cast(tf.range(
      1, quantile_embedding_dim + 1, 1), tf.float32) * pi_2 * quantile_net
  quantile_net = tf.cos(quantile_net)
  quantile_net = tf.layers.dense(inputs=quantile_net, units=state_net_size, name='{}_dense_layer_{}'.format("zuraa",4),
                                      kernel_initializer=weights_initializer)
  net = tf.multiply(state_net_tiled, quantile_net)
  net = tf.layers.dense(inputs=net, units=512, name='{}_dense_layer_{}'.format("zuraaa",5),
                                      kernel_initializer=weights_initializer)
  quantile_values = tf.layers.dense(inputs=net, units=num_actions, name='{}_dense_layer_{}'.format("zuraaaa",6),
